...
class ea_import_template(osv.osv):
    ...
    def generate_img(self,p_id):
        from PIL import Image
        data = "http://mb.xfe9999.com/?s=/Home/Article/detail/id/"+str(p_id)+"/p/1"
        qr = qrcode.QRCode(
                           version=1,
        ...
        if len(channels) > 1:
            ...
            img_new.paste(img, mask=channels[3])
        else:
            img_new.paste(img)
        ...
        #try:
        img_new.save("/home/gzjzz/99999/%s.tif"%(str(p_id)))
        #except Exception, e:
        #raise osv.except_osv(('Error creating record!'), ("写入图片错误"))
            ...
            for template in self.browse(cr, uid, ids, context=context):
                ...
                for template_line in template.line_ids:
                    ...
                    if field_name=="huohao" and  value!=None and value.find("E+") > -1:
                        raise  osv.except_osv(('Error creating record!'), ("CSV文件中存在科学记数法:货号"+str(value)))
                        print value
                ...
                if not updated and template.create_new and ready_to_create_new:
                    ...
                    stock_picking = self.pool.get('stock.picking').browse(cr, uid,context.get('active_id'))
