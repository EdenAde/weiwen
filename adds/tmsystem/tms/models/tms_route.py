...
class TmsRoute(models.Model):
    ...
    name = fields.Char('路线', size=64, required=True, index=True)
    ...
    distance = fields.Float(
        '距离 (mi./kms)', digits=(14, 4),
        help='Route distance (mi./kms)', required=True)
    buzhu = fields.Many2one('product.product','补助／天', required=True)
    ...
    tollstation_ids = fields.Many2many(
        'tms.route.tollstation', string="收费站")
