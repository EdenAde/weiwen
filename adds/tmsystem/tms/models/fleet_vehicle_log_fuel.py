...
class FleetVehicleLogFuel(models.Model):
    ...
    product_qty = fields.Float(string='升', default=1.0,)
    tax_amount = fields.Float(string='税',)
    price_total = fields.Float(string='总计')
    special_tax_amount = fields.Float(
        compute="_compute_special_tax_amount", string='IEPS')
    price_unit = fields.Float(
        compute='_compute_price_unit', string='单价')
    price_subtotal = fields.Float(
        string="小计", compute='_compute_price_subtotal')
    invoice_id = fields.Many2one(
        'account.invoice', string='发票', readonly=True)
