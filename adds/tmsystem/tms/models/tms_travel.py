...
class TmsTravel(models.Model):
    ...
    state = fields.Selection([
        ('draft', '草稿'), ('progress', '进行中'), ('done', '完成'),
        ('cancel', '取消'), ('closed', '关闭')],
        readonly=True, default='draft')
    route_id = fields.Many2one(
        'tms.route', '路线', required=True,
    ...
    travel_duration_real = fields.Float(
        ...
        string='实际天数', help="Travel Real duration in hours")
    ...
    #mile_efficiency_travel = fields.Float()

    mine_start = fields.Float(
        string="开始里程数")
    mine_stop = fields.Float(
        string="结束里程数")
    mine_efficiency = fields.Float("相差",compute="_compute_mine_efficiency_extraction")
    ...
    fuel_log_ids = fields.One2many(
        'fleet.vehicle.log.fuel', 'travel_id', string='燃油记录')
    advance_ids = fields.One2many(
        'tms.advance', 'travel_id', string='预付')
    ...
    expense_id = fields.Many2one(
        'tms.expense', '费用', readonly=True)
    event_ids = fields.One2many('tms.event', 'travel_id', string='Events')
    is_available = fields.Boolean(
        compute='_compute_is_available',
        string='是否可用')
    operating_unit_id = fields.Many2one('operating.unit', 'Operating Unit',default=3)
    ...
    partner_ids = fields.Many2many(
        'res.partner', string='客户', compute='_compute_partner_ids',
        store=True)

    @api.depends('mine_stop', 'mine_start')
    def _compute_mine_efficiency_extraction(self):
        for rec in self:
            rec.mine_efficiency = (
                rec.mine_stop - rec.mine_start)
