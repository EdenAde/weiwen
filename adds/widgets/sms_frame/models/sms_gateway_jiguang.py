...
class SmsGatewayTwilio(models.Model):
    ...
    def send_message(self, sms_gateway_id, from_number, to_number, sms_content, my_model_name='', my_record_id=0, media=None, queued_sms_message=None):
	    ...
	    #
         #
        #send the sms/mms
        payload = {'temp_id': format_from.encode('utf-8'), 'To': format_to.encode('utf-8'), 'Body': sms_content.encode('utf-8'), 'StatusCallback': base_url + "/sms/twilio/receipt"}
        ...
        base64_auth_string=base64.b64encode(sms_account.appKey+':'+sms_account.masterSecret)
        headers = {
        ...
        response_string = requests.post(
            "https://api.sms.jpush.cn/v1/messages/batch",
            headers=headers,
            data=payload)
...
class SmsAccountTwilio(models.Model):
    ...
    masterSecret = fields.Char(string='masterSecret')
    left_num = fields.Integer(string='剩余条数')
