...
class SchoolStandard(models.Model):
    ...
    def name_get(self):
        ...
        return [(rec.id, rec.standard_id.name + '[' + rec.name  +
                 ']班') for rec in self]
...
class StudentStudent(models.Model):
    ...
    def createsql(self,vals):
        for stu in vals:
            ...
            standard_ids = self.env['school.standard'].search(
                [('all_name', '=', stu.get('standard')), ('school_id', '=', stu.get('school_id'))])
            standard = None
            if not standard_ids:
            ...
            insert_id = self._cr.execute("insert into student_student (user_id,school_id,state,standard_id) VALUES(%s,%s,%s,%s) " % (user_new.id,stu.get('school_id'),'done',standard))
            print "insert_id"
            print insert_id
            self._cr.execute(
    ...
    user_id = fields.Many2one('res.users', 'User ID', ondelete="cascade",
                              required=True)
    student_name = fields.Char('学生姓名', related='user_id.name',
                               store=True, readonly=True)
    pid = fields.Char('系统编号', required=True, default=lambda obj:
...
#        if res and res.is_school_teacher:
        print "has_school and login"
        print vals.get('school') and (res.is_school_teacher or vals.get('is_school_teacher'))
        #print res.is_school_teacher


        if vals.get('school') and (res.is_school_teacher or vals.get('is_school_teacher')):
            school = self.env['school.school'].browse(vals.get('school'))
            user_vals = {'name': vals.get('name'),
                         #'login': self.barcode,
                         'login': vals.get('work_email', False) or self._context.get('login'),
                         'password': vals.get('work_email', '123456'),
                         'partner_id': self.id,
                         'barcode':vals.get('barcode'),
                         'company_id': school.company_id.id,
                         'company_ids': [(school.company_id.id,)]}
            # Create user
            ...
            print "user_user_school_teacher"
            print user_vals
    ...
    def write(self, vals):
        ...
        # creating user
        for rec in self:
            if rec.school and rec.is_school_teacher and not rec.user_id and not self._context.get('cron_action',False):
...
class StudentReminder(models.Model):
    ...
    stu_id = fields.Many2one('student.student', '学生姓名', required=True,
                             default=check_user)
    name = fields.Char('标题')
    date = fields.Date('日期')
    description = fields.Text('描述')
    color = fields.Integer('颜色等级', default=0)
    company_id = fields.Many2one('res.company', string='学校', required=True,
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        ...
        return super(StudentReminder, self).search(args, offset, limit, order, count)
