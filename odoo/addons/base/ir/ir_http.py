...
class IrHttp(models.AbstractModel):
    ...
    def _auth_method_user(cls):
        ...
        if not request.uid:
            _logger.info("start redirect")
            _logger.info(request.uid)
            #return werkzeug.utils.redirect('http://tsg.zyhanyuqi.com/web/login', 301)
            #werkzeug.utils.redirect('http://tsg.zyhanyuqi.com/web/login')
            #http.redirect_with_hash('/web/login')
            #return request.redirect('http://tsg.zyhanyuqi.com/web/login')
