...
class Session(http.Controller):
    ...
    def authenticate(self, db, login, password, base_location=None):
        ...
        # sql = "select is_first_login where id="+info.get('uid')
        # self._cr.execute(sql)
        # res = self._cr.fetchall()
        #
        # if(res[0]['is_first_login']==1):
        #     sql = "update res_users set is_first_login=0 where id=" + info.get('uid')
        #     self._cr.execute(sql)
        #     info["is_first_login"]=1
        # else:
        #     info["is_first_login"] = 0
        #
        # return info
...
class Binary(http.Controller):
    ...
    @http.route('/app/binary/upload_model_attachment/<string:model>', type='http', auth="public", csrf=False)
    @serialize_exception
    def upload_attachment3(self, model, file,**post):
        Model = request.env['ir.attachment']
    ...
    @http.route('/app/binary/upload_model_attachment/<string:model>/<int:id>', type='http', auth="public", csrf=False)
    @serialize_exception
    def upload_attachment2(self, model,id,file,**post):
        Model = request.env['ir.attachment']
    ...
    @http.route('/app/binary/upload_attachment/t', type='http', auth="none")
    @serialize_exception
    def app_upload_attachment2(self):

        return json.dumps({'file':1})


    @http.route('/app/binary/upload_attachment', type='http', auth="none")
    @serialize_exception
    def app_upload_attachment(self,ufile):
        Model = request.env['ir.attachment']
        out = """"""
        try:
            attachment = Model.create({
