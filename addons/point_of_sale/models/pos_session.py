...
class PosSession(models.Model):
    ...
    def create(self, values):
        ...
        if not pos_config.journal_id:
            default_journals = pos_config.with_context(ctx).sudo().default_get(['journal_id', 'invoice_journal_id'])
        ...
        if not pos_config.journal_ids:
            ...
            journals = Journal.with_context(ctx).sudo().search([('journal_user', '=', True), ('type', '=', 'cash')])
            if not journals:
                journals = Journal.with_context(ctx).sudo().search([('type', '=', 'cash')])
                if not journals:
                    journals = Journal.with_context(ctx).sudo().search([('journal_user', '=', True)])
