...
exports.PosModel = Backbone.Model.extend({
    ...
    the_gui_error:function(){
        // var self = this
        // alert(self.gui.show_popup)
        //
    ...
    models: [
    {
        ...
        model:  'res.partner',
        fields: ['name','street','city','state_id','country_id','vat','phone','zip','mobile','email','barcode','write_date','property_account_position_id','card_id','student_id','employee'],
        domain: [['card_id','!=',false],['company_id.id','=',session.company_id]],
        ...
        domain: [['sale_ok','=',true],['available_in_pos','=',true],['is_book','=',true],['school_id.company_id.id','=',session.company_id],['quantity','=',0]],
    ...
    load_new_partners: function(){
        console.log("load_new_partners")
        var self = this;
        var def  = new $.Deferred();
        var fields = _.find(this.models,function(model){ return model.model === 'res.partner'; }).fields;
        new Model('res.partner')
            .query(fields)
            .filter([['card_id','!=',false],['write_date','>',this.db.get_partner_write_date()]])
    ...
    add_new_order_with_client_over: function(client,datasets){


        var orders = this.get_order_list()
        for(var i in orders){
            var clientin = orders[i].get_client()

            if(clientin!=null&&clientin.id==client.id){
                this.set('selectedOrder', orders[i]);
                return orders[i]
            }
        }
        ...
        if(datasets.length>0){
            _.map(datasets,function (data) {
                var product = self.db.get_product_by_id(data);
       ...
       // console.log(this.get_orders())
        ...
        //creates a new empty order and sets it as the current order
    add_new_order_with_client: function(client,datasets){
        ...
        //         console.log(clientin)
        //         console.log(clientin.id)
        //         console.log(client.id)
        //         console.log(orders[i])
        //         console.log(orders[i].get_type())
        //         console.log(orders[i].type)
        //     }
        //
        //     if(clientin&&clientin.id==client.id&&orders[i].type=='return'){
        //         this.set('selectedOrder', clientin);
        //         return orders[i]
        //     }
        // }


        var order = new exports.Order({},{pos:this,type:'return',temporary:true});
        order.set_type('return')
        order.type='return'
        console.log("return")
        console.log(order)
        var self = this
    ...
    get_book_with_isbn:function (parsed_code) {
        console.log(parsed_code);
        var selectedOrder = this.get_order();
        var book = this.db.get_book_by_barcode(parsed_code.base_code);

        if(!book){
            return false;
        }
        //selectedOrder.add_product(book);
        return book;
    },
    show_book_intent:function (book) {
        return true
    },

      get_book_from_server_with_barcode: function(barcode){
    var p = new Promise(function(resolve, reject){
        new Model('product.product').call('get_book_date_from_server_with_barcode',[1, barcode]).then(function(book){
                   // self.pos.db.add_products([book]);
                    //return book
                    resolve(book);
    ...
    scan_product1:function (parsed_code) {
          var selfdb = this.db
          var selectedOrder = this.get_order();

          var product1 = this.db.get_product_by_barcode(parsed_code.base_code);
        ...
        console.log(product1)
          ...
          var p = new Promise(function(resolve, reject){
        new Model('product.product').call('get_book_date_from_server_with_barcode',[1, parsed_code.base_code]).then(function(product){
                 //   selfdb.add_products([product]);
    ...
    scan_product: function(parsed_code){
        ...
        this.get_book_from_server_with_barcode(parsed_code.base_code).then(function (product) {
    ...
    scan_product_with_intent: function(parsed_code,intent){
        this.intent = intent
        var selectedOrder = this.get_order();
        var product = this.db.get_product_by_barcode(parsed_code.base_code);
...
exports.Orderline = Backbone.Model.extend({
    initialize: function(attr,options){
        ...
        //     this.issue_state = true
        // }else if(this.product.availability=='notavailable'){
        //     this.issue_state = false
        // }
    ...
    init_from_JSON: function(json) {
        ...
        console.log("init_from_JSON_product")
    ...
    get_issue_html:function () {
       // var product = this.get_product();

        if(this.issue_state){
            return " &lt;span class='product-qty-low' &gt;借出&lt;/span&gt;";
        }else{
            return " &lt;span class='product-qty' >在馆!&lt;/span&gt;";
        }
    },
    ...
    get_quantity_str_with_unit: function(){
        ...
        if(unit && !unit.is_unit){
            ...
            return Math.floor(this.quantityStr);
        }
    },
    ...
    get_issue_state:function () {
        return this.issue_state
    },

    // selects or deselects this orderline
    set_issue_state: function(state){
    ...
    export_as_JSON: function() {
        ...
        return {
            ...
            issue_state:this.get_issue_state(),
...
exports.Order = Backbone.Model.extend({
    ...
    remove_orderline_all: function(){
        //this.assert_editable();
        var self = this
        var lines = this.get_orderlines()
       // if(lines.length>0){
           // self.selected_orderline.set_selected(false)
            console.log("this.orderlines.models")
           console.log(this.orderlines)
            ...
            for (var i in lines){
                ...
                this.orderlines.remove(lines[i])
                // console.log()
                // console.log(lines[i])

               //var result = lines[i].set_quantity('remove')
            }
            }
           ...
           //  }
            this.trigger('change',this);
            console.log(this.get_orderlines())
            // this.selected_orderline.set_selected(false)
            // this.selected_orderline = undefined;
            // _.map(this.orderlines.models,function (orderline) {
            //     //console.log(orderline)
            //    // console.log(orderline.get_product())
            //     //self.orderlines.remove(orderline);
            //     orderline.set_quantity('remove')
            //
            // })
            // console.log(this.orderlines.models.length)
          //  this.selected_orderline = undefined;
       // }
       // console.log("this.orderlines.models2222")
       // console.log(this.orderlines.models)
       // this.select_orderline(this.get_last_orderline());
        // self.deselect_orderline();
       //  self.selected_orderline = undefined;
       //  console.log("self.selected_orderline")
       //  console.log(self.selected_orderline)
    },
