...
class StockMove(models.Model):
    ...
    product_id = fields.Many2one(
        ...
        domain=[('type', 'in', ['product', 'consu'])],ondelete='cascade', index=True, required=True,
        states={'done': [('readonly', True)]})
    ...
    def _move_quants_by_lot(self, ops, lot_qty, quants_taken, false_quants, lot_move_qty, quant_dest_package_id):
        ...
        """
        return self.browse(lot_move_qty.keys())._move_quants_by_lot(quants_taken, false_quants, ops, lot_qty, lot_move_qty, quant_dest_package_id)

    @api.multi
    def _move_quants_by_lot(self, quants_taken, false_quants, pack_operation, lot_quantities, lot_move_quantities, dest_package_id):
        Quant = self.env['stock.quant']
