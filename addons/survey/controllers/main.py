...
class WebsiteSurvey(http.Controller):
    ...
    @http.route(['/api/survey/<int:id>/<int:eid>'],
                type='http', auth='public', website=True)
    def survey_api(self, id,eid,**post):
        '''Display and validates a survey'''
        employee = request.env['hr.employee'].sudo().browse(eid)
        survey = request.env['survey.survey'].sudo().browse(int(id))
        ...
        questions = request.env['survey.question']
        labels = request.env['survey.label']


        res_qs = []
        questions = questions.sudo().search([('survey_id', '=', id)])
        for question in questions:
            ...
            question_dic['title'] = question.question


            if question.type == 'simple_choice':
                question_dic['type'] = "select"
                tag = '%s_%s_%s' % (question.survey_id.id, question.page_id.id, question.id)
                question_dic['name'] = tag
                labels = labels.sudo().search([('question_id', '=', question.id)])
                lbs = []
                for label in labels:
                    lbs.append({"label": label.value, 'value': label.id})
                    #lbs.append(label.value)
                    # lab = {'id':label.id,'title':label.value,'sequence':label.sequence}
                question_dic['items'] = lbs

            if question.type == 'free_text':
                tag = '%s_%s_%s' % (question.survey_id.id, question.page_id.id, question.id)
                question_dic['name'] = tag
                question_dic['type'] = "textarea"

            if question.type == 'textbox':
                tag = '%s_%s_%s' % (question.survey_id.id, question.page_id.id, question.id)
                question_dic['name'] = tag
                question_dic['type'] = "text"
                question_dic['detailType'] = "text"
            if question.type == 'numerical_box':
                tag = '%s_%s_%s' % (question.survey_id.id, question.page_id.id, question.id)
                question_dic['name'] = tag
                question_dic['type'] = "text"
                question_dic['detailType'] = "int"
            if question.type == 'datetime':
                tag = '%s_%s_%s' % (question.survey_id.id, question.page_id.id, question.id)
                question_dic['name']=tag
                question_dic['type'] = "text"
                question_dic['detailType'] = "standardDate"
            # if question.type == 'matrix':
            #     question_dic['type'] = "text"
            #     question_dic['detailType'] = "standardDate"

            if question.type == 'multiple_choice':
                question_dic['type'] = "checkboxs"
                labels = labels.sudo().search([('question_id', '=', question.id)])
                lbs = []
                for label in labels:
                    tag = '%s_%s_%s_%s' % (question.survey_id.id, question.page_id.id, question.id,label.id)
                    lbs.append({"label":label.value,'value':label.id,'tag':tag})
                    # lab = {'id':label.id,'title':label.value,'sequence':label.sequence}
                question_dic['items'] = lbs
            if question.type == 'matrix':
                question_dic['type'] = "radios"
                question_dic['detailType'] = "matrix"
                labels = labels.sudo().search([('question_id', '=', question.id)])
                lbs = []
                for label in labels:
                    tag = '%s_%s_%s_%s' % (question.survey_id.id, question.page_id.id, question.id, label.id)
                    lbs.append({"label": label.value, 'value': label.id, 'tag': tag})
                    #lbs.append(label.value)
                    #lbs.append({"label": label.value, 'id': label.id})
                    # lab = {'id':label.id,'title':label.value,'sequence':label.sequence}
                question_dic['items'] = lbs
                labels2 = labels.sudo().search([('question_id_2', '=', question.id)])
                lbs2 = []
                for label in labels2:
                    tag = '%s_%s_%s_%s' % (question.survey_id.id, question.page_id.id, question.id, label.id)
                    lbs2.append({"label": label.value, 'value': label.id, 'tag': tag})
                    #lbs2.append(label.value)
                    # lab = {'id':label.id,'title':label.value,'sequence':label.sequence}
                question_dic['items2'] = lbs2
                ...
                #     lbs.append(label.value)
                question_dic['items'] = lbs
            res_qs.append(question_dic)

        res_trival['content'] += res_qs
    ...
    @http.route(['/api/survey/partner'], type='http', methods=['POST'], auth='public',
                website=True)
    def new_partner(self, **post):
        p = {
            "name":post.get("name"),
            "address":post.get("name"),
            "phone": post.get("phone"),
            "image":post.get("avatar"),
            "customer":False
        }
        newp = request['res.partner'].create(p)
        return json.dumps(newp)


    # AJAX submission of a page
    @http.route(['/api/survey/submit'], type='http', methods=['POST','GET'], auth='public',
                )
    def api_submit(self, **post):
        ...
        newp = request.env['res.partner'].create(p)
        #_logger.info(newp)
        survey_id = int(post['survey_id'])
        vals1 = {'survey_id':survey_id}
        #if request.website.user_id != request.env.user:
        vals1['partner_id'] = newp.id
        vals1['location'] = post.get('survey_location')
        vals1['user_id'] = post.get('user_id')
        vals1['lang'] = post.get('zh_CN')
        vals1['identification_id'] = post.get('idcard')
        user_input = UserInput.create(vals1)
        ...
        errors = {}
        for question in questions:
            answer_tag = "%s_%s_%s" % (survey_id, question.page_id.id, question.id)
        ...
        if len(errors):
            ...
            # try:
            #     user_input = request.env['survey.user_input'].sudo().search([('token', '=', post['token'])],
            #                                                                 limit=1)
            ...
            for question in questions:
                ...
                try:
                    request.env['survey.user_input_line'].sudo().save_lines(user_input.id, question, post,answer_tag)
                except KeyError:  # Invalid token
