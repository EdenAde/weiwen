...
class account_analytic_invoice_line(models.Model):
    ...
    def _amount_line(self):
        ...
        for line in self:
            ...
            if line.analytic_account_id.pricelist_id:
                ...
                res[line.id] = self._env['res.currency'].round(res[line.id])
        return res
    ...
    price_subtotal=fields.Float(compute="_amount_line", string='Sub Total', digits_compute= dp.get_precision('Account'))
    ...
    def product_id_change(self,product, uom_id, qty=0, name='', partner_id=False, price_unit=False, pricelist_id=False, company_id=None, context=None):
        ...
        if partner_id:
            part = self._env['res.partner'].browse(partner_id)
            if part.lang:
                local_context.update({'lang': part.lang})

        result = {}
        res = self._env['product.product'].browse(product)
        price = False
        ...
        if not name:
            name = self._env['product.product'].name_get([res.id])[0][1]
